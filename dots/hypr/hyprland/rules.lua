...
-- Quickshell: panel family layer rules (ii sidebar / lunae / akebono)
hl.layer_rule({ match = { namespace = "quickshell:sidebarRight:left" }, animation = "slide left" })

-- LUNAE_BLUR marker is load-bearing: lunae Drawers.qml seds it to toggle blur
hl.layer_rule({ match = { namespace = "quickshell:lunaeDrawers" }, animation = "fade" })
hl.layer_rule({ match = { namespace = "quickshell:lunaeDrawers" }, blur = false }) -- LUNAE_BLUR
hl.layer_rule({ match = { namespace = "quickshell:lunaeDrawers" }, ignore_alpha = 0.2 })

-- Surfaces that animate in QML: hyprland must not popin-scale or slide them
for _, ns in ipairs({
	"quickshell:lunaeBarExclusion",
	"quickshell:lunaeFrameTop",
	"quickshell:lunaeFrameRight",
	"quickshell:lunaeFrameBottom",
	"quickshell:lunaeHubHotZone",
	"quickshell:lunaeSidebarHotZone",
	"quickshell:akebonoRunner",
	"quickshell:akebonoGlyphPicker",
	"quickshell:akebonoSheetRunner",
	"quickshell:akebonoOverview",
	"quickshell:akebonoDesktopMenu",
	"quickshell:panelFamilyPicker",
}) do
	hl.layer_rule({ match = { namespace = ns }, no_anim = true })
end

for _, family in ipairs({ "akebono" }) do
	hl.layer_rule({ match = { namespace = "quickshell:" .. family .. "Shelf" }, animation = "slide bottom" })
	hl.layer_rule({ match = { namespace = "quickshell:" .. family .. "ShelfTop" }, animation = "slide top" })
end
