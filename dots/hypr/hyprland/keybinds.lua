...
hl.bind(
	...
	{ locked = true, repeating = true, description = "Misc: Brightness up" }
)
hl.bind(
	"XF86MonBrightnessDown",
	hl.dsp.exec_cmd(qsIpcCall .. " brightness decrement || brightnessctl s 5%-"),
	{ locked = true, repeating = true, description = "Misc: Brightness down" }
)
hl.bind(
	"XF86AudioRaiseVolume",
	hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%+ -l 1.5"),
	{ locked = true, repeating = true, description = "Misc: Volume up" }
)
hl.bind(
	"XF86AudioLowerVolume",
	hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%-"),
	{ locked = true, repeating = true, description = "Misc: Volume down" }
)
...
hl.bind(
	"CTRL + SUPER + P",
	hl.dsp.global("quickshell:panelFamilyPicker"),
	{ description = "Shell: Panel family picker" }
)
hl.bind("SUPER + H", hl.dsp.global("quickshell:hubToggle"), { description = "Shell: Toggle hub (lunae)" })
hl.bind(
	"SUPER + SHIFT + V",
	hl.dsp.global("quickshell:regionTranslate"),
	{ description = "Utilities: Region translate" }
)
...
for i = 1, 6 do
	...
	hl.bind(
		...
		i == 1 and { description = "Window: Focus in direction (arrows)" } or nil
	)
end
for i = 1, 4 do
	...
	hl.bind(
		...
		i == 1 and { description = "Window: Move in direction (arrows)" } or nil
	)
end
...
hl.bind(
	...
	{ repeating = true, description = "Window: Adjust split ratio (; and ')" }
)
...
for i = 1, 10 do
	...
	end, i == 1 and { description = "Window: Send to workspace 1-10" } or nil)
end
for i = 1, 10 do
...
for i = 1, 10 do
	...
	end, i == 1 and { description = "Workspace: Focus workspace 1-10" } or nil)
end
for i = 1, 10 do
	local numpadkey = { 87, 88, 89, 83, 84, 85, 79, 80, 81, 90 }
...
for i = 1, 4 do
	...
	local desc = { "Workspace: Focus right", "Workspace: Focus left" }
	hl.bind(keycombos[i], hl.dsp.focus({ workspace = prefix[i] .. "1" }), desc[i] and { description = desc[i] } or nil)
end
for i = 1, 4 do
...
for i = 1, 4 do
	...
	local keycombos = { key[1], key[2], "CTRL + " .. key[1], "CTRL + " .. key[2] }
	local prefix = { "+", "-", "r+", "r-" }
	local desc = { "Workspace: Focus right (scroll)", "Workspace: Focus left (scroll)" }
	hl.bind(keycombos[i], hl.dsp.focus({ workspace = prefix[i] .. "1" }), desc[i] and { description = desc[i] } or nil)
end
