...
Scope { // Scope
    ...
    Variants {
        ...
        PanelWindow {
            ...
            property bool reveal: root.pinned || (Config.options?.dock.hoverToReveal && dockMouseArea.containsMouse) || dockApps.requestDockShow || (Config.options?.dock.smartHide ? !dockOcclusion.occluded : !ToplevelManager.activeToplevel?.activated)

            WindowOcclusion {
                id: dockOcclusion
                screen: dockRoot.modelData
                region: Qt.rect(
                    dockRoot.modelData.x + (dockRoot.modelData.width - dockVisualBackground.width) / 2,
                    dockRoot.modelData.y + dockRoot.modelData.height - Appearance.sizes.hyprlandGapsOut - dockVisualBackground.height,
