...
RowLayout {
    ...
    property var searchPrefixType: {
        ...
        if (p.action && root.searchingText.startsWith(p.action)) return SearchBar.SearchPrefixType.Action;
        if (p.app && root.searchingText.startsWith(p.app)) return SearchBar.SearchPrefixType.App;
        if (p.clipboard && root.searchingText.startsWith(p.clipboard)) return SearchBar.SearchPrefixType.Clipboard;
        if (p.emojis && root.searchingText.startsWith(p.emojis)) return SearchBar.SearchPrefixType.Emojis;
        if (p.kaomojis && root.searchingText.startsWith(p.kaomojis)) return SearchBar.SearchPrefixType.Kaomojis;
        if (p.math && root.searchingText.startsWith(p.math)) return SearchBar.SearchPrefixType.Math;
        if (p.shellCommand && root.searchingText.startsWith(p.shellCommand)) return SearchBar.SearchPrefixType.ShellCommand;
        if (p.symbols && root.searchingText.startsWith(p.symbols)) return SearchBar.SearchPrefixType.Symbols;
        if (p.webSearch && root.searchingText.startsWith(p.webSearch)) return SearchBar.SearchPrefixType.WebSearch;
    ...
    MaterialShapeWrappedMaterialSymbol {
        ...
        shape: switch(root.searchPrefixType) {
            ...
            case SearchBar.SearchPrefixType.Kaomojis: return MaterialShape.Shape.Flower;
            ...
            case SearchBar.SearchPrefixType.Symbols: return MaterialShape.Shape.Pentagon;
        ...
        text: switch (root.searchPrefixType) {
            ...
            case SearchBar.SearchPrefixType.Kaomojis: return "sentiment_very_satisfied";
            case SearchBar.SearchPrefixType.Math: return "calculate";
            case SearchBar.SearchPrefixType.ShellCommand: return "terminal";
            case SearchBar.SearchPrefixType.Symbols: return "special_character";
