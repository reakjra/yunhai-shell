...
Item {
    ...
    property var allCommands: [
        ...
        {
            name: "key",
            description: Translation.tr("Set, view, or clear API key"),
            execute: (args) => {
                if (args.length < 2) {
                    Booru.addSystemMessage(Translation.tr("Usage:\n/key danbooru <profile_url>\n/key gelbooru <user_id> <api_key>\n/key <provider> get — view stored key\n/key <provider> clear — remove key"))
                    return
                }
                const provider = args[0].toLowerCase()
                switch (args[1]) {
                    case "get": Booru.getBooruKey(provider); break
                    case "clear": Booru.clearBooruKey(provider); break
                    default: Booru.setBooruKey(provider, args.slice(1))
                }
            }
        },
    ]
