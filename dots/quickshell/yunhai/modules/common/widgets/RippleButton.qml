...
Button {
    ...
    property color buttonColor: {
        const hoverCol = root.toggled ? colBackgroundToggledHover : colBackgroundHover
        const baseCol = root.toggled ? colBackgroundToggled : colBackground
        if (root.hovered) return hoverCol
        // match hover RGB when base is transparent to prevent dark flash in color anim
        return baseCol.a < 0.01
            ? Qt.rgba(hoverCol.r, hoverCol.g, hoverCol.b, 0)
            : baseCol
    }
