...
ContentPage {
    ...
    Loader {
        ...
        active: SettingsStyle.grouped
        visible: active
        source: active ? Quickshell.shellPath(`modules/${Config.options.panelFamily}/settings/sections/InterfaceSections.qml`) : ""
    }
    ...
    ContentSection {
        ...
        ContentSubsection {
            title: Translation.tr("Annotation tool")

            MaterialTextArea {
                Layout.fillWidth: true
                placeholderText: "swappy -f %f"
                text: Config.options.regionSelector.annotation.command
                wrapMode: TextEdit.Wrap
                onTextChanged: {
                    Config.options.regionSelector.annotation.command = text;
                }
            }
        }
    }
    ...
    ContentSection {
        ...
        ConfigSwitch {
            ...
            text: Translation.tr('Use DeepL (Free)')
            enabled: Config.options.sidebar.translator.enable
            checked: Config.options.sidebar.translator.useDeepL
            onCheckedChanged: {
                Config.options.sidebar.translator.useDeepL = checked;
                if (checked && !KeyringStorage.loaded) KeyringStorage.fetchKeyringData();
            }
            StyledToolTip {
                text: Translation.tr("Use DeepL Free API instead of the trans CLI tool.\nGet your API key at deepl.com/your-account/keys")
            }
        }
    ...
    ContentSection {
        ...
        ConfigSwitch {
            ...
            text: Translation.tr("Classic overview")
            checked: Config.options.family?.overview.classic ?? false
            onCheckedChanged: Config.options.family.overview.classic = checked
            StyledToolTip {
                text: Translation.tr("The ii/lunae centered workspace grid instead of the desktop-family expose")
            }
        }

        ColumnLayout {
            visible: !SettingsStyle.grouped || (Config.options.family?.overview.classic ?? false)
            Layout.fillWidth: true
            spacing: 4
    ...
    ContentSection {
        ...
        MaterialTextArea {
            Layout.fillWidth: true
            placeholderText: Translation.tr("Wallpaper folder (selector shortcut, and where Konachan / osu! / Wallhaven downloads go)")
            text: Config.options.wallpaperSelector.downloadPath
            wrapMode: TextEdit.Wrap
            onTextChanged: {
                Config.options.wallpaperSelector.downloadPath = text;
            }
        }
    }
