...
ContentPage {
    ...
    ContentSection {
        ...
        ConfigSwitch {
            buttonIcon: "lock"
            text: Translation.tr("Preserve overrides on wallpaper/scheme change")
            checked: ColorOverrideStore.data.preserveOnWallpaperChange
            onCheckedChanged: {
                ColorOverrideStore.data.preserveOnWallpaperChange = checked;
            }
            StyledToolTip {
                text: Translation.tr("When disabled, color overrides are reset when switching wallpaper or Material scheme")
            }
        }
