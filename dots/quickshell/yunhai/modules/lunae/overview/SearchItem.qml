...
RippleButton {
    ...
    readonly property bool isFavourited: isApp && root.entry?.id && LauncherApps.isPinned(root.entry.id)
    readonly property string subtitle: isApp ? (root.entry?.comment || root.entry?.genericName || "") : ""

    signal rightClicked(LauncherSearchResult entry, real mouseX, real mouseY)
    ...
    altAction: (event) => {
        if (root.isApp && root.entry) {
            const mapped = root.mapToItem(null, event.x, event.y);
            root.rightClicked(root.entry, mapped.x, mapped.y);
        }
    }
