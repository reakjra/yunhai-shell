...
Singleton {
    ...
    function refreshCountdown() {
        ...
        if (countdownSecondsLeft === 0 && Persistent.states.timer.countdown.running) {
            // Timer finished
            Persistent.states.timer.countdown.running = false;

            // Send notification
            Quickshell.execDetached(["notify-send", "-i", "alarm-clock", "Countdown", Translation.tr("Countdown finished!"), "-a", "Shell"]);
            if (Config.options.sounds.countdown) {
                Audio.playSystemSound("alarm-clock-elapsed");
            }
        }
    }
    ...
    function toggleCountdown() {
        if (Persistent.states.timer.countdown.running) {
            Persistent.states.timer.countdown.running = false;
        } else if (countdownSecondsLeft > 0) {
            // Resume
