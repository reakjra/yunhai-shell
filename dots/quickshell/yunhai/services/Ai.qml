...
Singleton {
    ...
    property bool namingInProgress: false // auto-naming in progress
    ...
    property var currentChatMetadata: {}
    ...
    property var models: Config.options.policies.ai === 2 ? {} : {
        ...
        "deepseek-deepseek-r1-distill-llama-70b-free": aiModelComponent.createObject(this, {
        ...
        "gemini-3-pro-preview": aiModelComponent.createObject(this, {
            ...
            "icon": "google-gemini-symbolic",
            "api_format": "openai",
            "description": "Proxy Gemini personalizzato",
            "homepage": "https://lorebary.sophiamccarty.com/aistudio",
            "endpoint": "https://lorebary.sophiamccarty.com/aistudio",
    ...
    function saveCurrentChat() {
        ...
        chatSaveFile.setText(saveContent);
        chatSaveFile.chatName = ""; // unload chatSaveFile or else it breaks
    }

    function autoNameAndSave() {
        const chatMessages = root.chatToJson();
        const summary = chatMessages
            .filter(m => m.role === "user" || m.role === "assistant")
            .map(m => `${m.role}: ${(m.rawContent ?? "").substring(0, 200)}`)
            .slice(0, 6)
            .join("\n");

        root.namingInProgress = true;
        autoNameProc.requestBody = JSON.stringify({
            contents: [{ parts: [{ text: "Analyze this conversation and respond with ONLY a valid JSON object containing \"title\" (short 4-5 word summary) and \"icon\" (Material Symbols icon name like \"lightbulb\", \"code\", \"chat\"). Use the conversation's language. No markdown, no extra text.\n\nConversation:\n" + summary }] }],
            generationConfig: { temperature: 0.2 }
        });
        autoNameProc.running = true;
    }
    ...
    function loadData(data) {
        try {
            ...
            // Handle both old format (flat array) and new format ({metadata, messages})
            const messages = Array.isArray(saveData) ? saveData : saveData.messages;
    ...
    Process {
        ...
        property string apiKey: root.apiKeys ? (root.apiKeys["gemini"] ?? "") : ""
        command: ["curl", "-s",
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                root.namingInProgress = false;
                root.addMessage(Translation.tr("Auto-naming failed (request error). Save manually with /save NAME"), root.interfaceRole);
            }
        }
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    if (response.error) {
                        root.addMessage(Translation.tr("Auto-naming failed: %1\nSave manually with /save NAME").arg(response.error.message ?? "Unknown error"), root.interfaceRole);
                        return;
                    }
                    ...
                    Qt.callLater(() => {
                        root.saveChat(result.title, { icon: result.icon });
                    });
                } catch (e) {
                    console.log("[AI] Auto-naming failed:", e, text);
                    root.addMessage(Translation.tr("Auto-naming failed (bad response). Save manually with /save NAME"), root.interfaceRole);
                }
            }
        }
    }
