...
shellConfigPath="$HOME/.config/yunhai/global.json"
customDownloadPath=$(jq -r '.wallpaperSelector.downloadPath // empty' "$shellConfigPath" 2>/dev/null)
customDownloadPath="${customDownloadPath/#\~/$HOME}"
WALLPAPER_DIR="${customDownloadPath:-$PICTURES_DIR/Wallpapers}"
