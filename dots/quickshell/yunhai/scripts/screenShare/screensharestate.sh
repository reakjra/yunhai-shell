...
# Kill any older instances of this script to prevent process leaking
for pid in $(pgrep -f "screensharestate.sh"); do
    if [ "$pid" != "$$" ] && [ "$pid" != "$PPID" ]; then
        kill -9 "$pid" 2>/dev/null
    fi
done

STATE_FILE="$HOME/.local/state/quickshell/yunhai/user/generated/screenshare/apps.txt"
